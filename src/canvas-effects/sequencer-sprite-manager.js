...
class ManagedTextureSprite extends ManagedSprite {
    view;
    constructor({ asset, tiling = false }) {
        super(asset);
...
class ManagedAnimatedSprite extends ManagedSprite {
    ...
    constructor({ asset, tiling = false }) {
        ...
        if (asset instanceof VideoAsset) {
            ...
            this.view = view;
            this.controls = new SpritePlaybackControls(view, asset.framerate, asset.frameObjects.length);
        }
    }
    destroy() {
        this.stop();
...
export class SequencerSpriteManager extends PIXI.Container {
    ...
    get tileScale() {
        return this.#activeSprite?.view instanceof PIXI.TilingSprite ? this.#activeSprite?.view.tileScale : undefined;
    }
    set tileScale(point) {
        if (this.#activeSprite?.view instanceof PIXI.TilingSprite) {
            this.#activeSprite.view.tileScale.copyFrom(point);
        }
    }
    get tilePosition() {
        return this.#activeSprite?.view instanceof PIXI.TilingSprite
            ? this.#activeSprite?.view.tilePosition
            : undefined;
    }
    ...
    get width() {
        return (this.#activeSprite?.view.width ??
    ...
    get height() {
        return (this.#activeSprite?.view.height ??
    ...
    async #preloadVariants() {
        ...
        const sprites = await Promise.all(allFiles.map(async (filePath) => {
    ...
    #applyTextSpriteValues(sprite) {
        const otherTextSprite = Object.values(this.#managedSprites).find((s) => s !== sprite && s instanceof ManagedTextSprite);
