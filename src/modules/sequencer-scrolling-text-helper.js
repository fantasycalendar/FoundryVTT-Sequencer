...
export default class SequencerScrollingTextHelper {
  ...
  static play(data){
    ...
    const position = source instanceof PlaceableObject
    ...
    if (source && multiplier) {
      let randomOffset = canvaslib.get_random_offset(source, multiplier, twister);
    ...
    if (extraOffset) {
      ...
      if (extraOffset.local) {
        newOffset = canvaslib.rotateAroundPoint(0, 0, newOffset.x, newOffset.y, source?.rotation ?? 0);
      }
      position.x -= newOffset.x;
      position.y -= newOffset.y;
    }

    return canvas.interface.createScrollingText(position, data.content, data.options);
