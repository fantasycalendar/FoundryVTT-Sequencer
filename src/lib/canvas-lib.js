...
export function get_token_scale_adjustment(tokenDocument) {

	const {width: textureWidth, height: textureHeight} = tokenDocument.object.texture;
	...
	let scaleCorrection = tokenDocument.ring.subject.scale;

	let scalePadding = 0;

	let {width: tokenWidth, height: tokenHeight} = tokenDocument.getSize();
