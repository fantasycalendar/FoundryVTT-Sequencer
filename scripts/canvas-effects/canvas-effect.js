...
import PerspectiveMesh from "../lib/perspective-mesh.js";
...
export default class CanvasEffect extends PIXI.Container {
	...
	getSourceData() {
		...
		const rotation = this.source instanceof MeasuredTemplate && this.sourceDocument?.t !== "rect"
			...
			: this.sourceDocument?.rotation && this.sourceDocument?.t !== "rect" ? Math.normalizeRadians(Math.toRadians(this.sourceDocument?.rotation)) : 0;
	...
	_initializeVariables() {
		...
		this.texturedSprite = this.sprite;
	...
	async _transformSprite() {
		...
		if (this.data.stretchTo) {
			...
			if (!this.texturedSprite.texture.valid && this._texture?.valid) {
				if (this.data.perspectiveRatio !== 1) {
					const perspectiveMesh = new PerspectiveMesh(this._texture, this.data.perspectiveRatio);
					perspectiveMesh.position.set(
						perspectiveMesh.x - this._texture.width / 2,
						perspectiveMesh.y - this._texture.height / 2
					);
					this.texturedSprite = this.sprite.addChild(perspectiveMesh);
				}else {
					this.texturedSprite.texture = this._texture;
				}
			}
		...
		if (this.video && (this._startTime || this._loopOffset > 0) && this.video?.currentTime !== undefined) {
			await lib.wait(20)
			this.texturedSprite.texture.update();
		}
...
class PersistentCanvasEffect extends CanvasEffect {
	...
	async _startEffect() {
		...
		this.video.pause();
		this.video.currentTime = this._endTime;
		if (this.texturedSprite.texture) {
			setTimeout(() => {
				this.texturedSprite.texture.update();
			}, 350);
		}
