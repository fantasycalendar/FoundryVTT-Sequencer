...
- *Effects* - Fixed `.screenSpaceAboveUI()` not correctly setting its renderable status when effects were played on its layer
- *Effects* - Fixed `.shape()` not being affected by `.fadeIn()`, `.fadeOut()`, or `.opacity()`
